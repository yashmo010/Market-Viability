...
def _params_for(cols) -> dict:
    """XGB_PARAMS plus monotone constraints for whichever constrained features are present."""
    constraints = {c: MONOTONE_BY_FEATURE[c] for c in cols if c in MONOTONE_BY_FEATURE}
...
def train_final(X: pd.DataFrame, y: np.ndarray, n_estimators: int,
                out_path: Path, params: dict | None = None) -> xgb.XGBRegressor:
    params = {**(params or XGB_PARAMS), "n_estimators": max(n_estimators, 50)}
    model = xgb.XGBRegressor(**params)
...
def train_product_type(product_type: str, report: dict) -> None:
    ...
    full_params = _params_for(X.columns)
    cv_full = cross_validate(X, y, categories, f"{product_type} FULL", params=full_params)
    model_full = train_final(X, y, cv_full["best_iteration"],
                             MODELS_DIR / f"xgb_{product_type}.json", params=full_params)
...
def main() -> None:
    ...
    report: dict = {
        ...
        "monotone_constraints": MONOTONE_BY_FEATURE,  # full model only; aspects-only drops price
