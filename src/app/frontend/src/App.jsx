...
export default function App() {
  ...
  const [specSchema, setSpecSchema] = useState({ categories: {} });
  ...
  // Category-specific structured specs (schema-driven) + company/product maturity
  const [structuredSpecs, setStructuredSpecs] = useState({});
  const [companyMaturity, setCompanyMaturity] = useState('established');
  const [productMaturity, setProductMaturity] = useState('iterated');
  ...
  // Fetch the schema-driven category spec fields on mount
  useEffect(() => {
    fetch('/api/spec_schema')
      .then(res => (res.ok ? res.json() : { categories: {} }))
      .then(data => setSpecSchema(data || { categories: {} }))
      .catch(err => console.error('Error loading spec schema:', err));
  }, []);

  // Reset structured specs whenever the category changes (fields differ per category)
  useEffect(() => {
    setStructuredSpecs({});
  }, [selectedCategory]);
  ...
  // Before the user runs anything, show the form full-page so every block is
  // visible; once Predict/Review is triggered, collapse to the results layout.
  const hasStarted = isLoadingCoach || isLoadingPredict || coachResult || predictResult;
  ...
  const simulatorForm = (
    <SimulatorForm
      ...
      setStructuredSpecs={setStructuredSpecs}
  ...
  return (
    <div style={styles.app}>
      ...
      <main style={styles.main}>
        {/* VIEW 1: SIMULATOR — full-page form until a run starts */}
        {activeTab === 'simulator' && !hasStarted && (
          <div style={styles.simulatorFull}>
            {simulatorForm}
          </div>
        )}

        {/* VIEW 1: SIMULATOR — two-column layout once a run has started */}
        {activeTab === 'simulator' && hasStarted && (
