    ...
    <div style={styles.chartWrapper} className="spotlight-card interactive-transition">
      ...
      <svg id={chartId} viewBox={`0 0 ${chartWidth} ${chartHeight}`} width="100%" height={chartHeight} style={styles.svg}>
        ...
        {aspects.map((aspect, idx) => {
          ...
          // Below the category average → red; at/above → green.
          const perfColor = score >= avg ? 'var(--success)' : 'var(--danger)';
          ...
          {aspects.map((aspect, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Magnetic Spoke Marker Circle (red if below category average) */}
                {isHighlighted && (
                  <circle
                    ...
                    r="5"
                    fill={(data[aspect]?.score ?? data[aspect] ?? 0) >= (categoryAvg[aspect] ?? 0) ? 'var(--success)' : 'var(--danger)'}
                    stroke="var(--bg-panel)"
                    strokeWidth="1.5"
                  />
                )}
