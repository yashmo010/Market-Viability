...
import { CategorySpecFields, MaturityBlock } from './SpecBlocks';
...
export default function SimulatorForm({
  ...
  // Schema-driven, category-specific spec fields
  const categorySchema = specSchema?.categories?.[selectedCategory];
  const specFields = categorySchema?.fields || [];
  const categoryLabel = categorySchema?.label || selectedCategory.replace(/_/g, ' ');
  const handleSpecChange = (key, value) => {
    setStructuredSpecs((prev) => ({ ...prev, [key]: value }));
  };

  // Allow submission when there's ANY input (description OR structured specs);
  // the server-side sufficiency gate decides whether it's rich enough to predict.
  const hasStructuredInput = Object.values(structuredSpecs || {})
    .some((v) => v !== '' && v != null && v !== 'None');
  const hasAnyInput = description.trim().length > 0 || hasStructuredInput;
  ...
  return (
    <div style={fullWidth ? styles.containerFull : styles.container}>
      <div style={styles.formSection}>
        ...
        <div style={styles.inputGroup}>
          <div style={styles.labelRow}>
            <label style={styles.label}>Custom Specifications &amp; Description</label>
          ...
          <span style={styles.charCount}>
            Free-form — combined with the category fields below.
          </span>
        </div>
        ...
        <div style={styles.btnRow}>
          <button
            type="button"
            onClick={onCoach}
            disabled={isLoadingCoach || isLoadingPredict || !hasAnyInput}
      ...
      <div style={fullWidth ? styles.rightGroup : styles.contents}>
        <CategorySpecFields
          categoryLabel={categoryLabel}
          fields={specFields}
          values={structuredSpecs}
          onChange={handleSpecChange}
        />

        <MaturityBlock
          companyMaturity={companyMaturity}
          setCompanyMaturity={setCompanyMaturity}
          productMaturity={productMaturity}
          setProductMaturity={setProductMaturity}
        />
      </div>

      {showCategoryTips && selectedCategory && (
        <div style={fullWidth ? { ...styles.tipsSection, gridColumn: '1 / -1' } : styles.tipsSection}>
...
const styles = {
  ...
  // Full-page layout (before Predict/Review is clicked): form left, category
  // specs + maturity right, category-intelligence tips spanning the bottom.
  containerFull: {
    display: 'grid',
